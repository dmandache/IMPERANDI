...
| `register` | `<input>_registered.csv` | `register_errors.csv`, `register_qc.csv`, one JSONL log per group, checkpoint/state files | Native-space canonical `mask_*` paths, original `source_mask_*` paths, registration status/QC fields, and native organ/tumor artifact paths |
