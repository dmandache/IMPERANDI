...
def test_missing_modality_is_never_silently_routed_to_ct():
    df = pd.DataFrame({"patient_id": ["p1"], "volume_id": ["v1"]})
...
def test_router_rejects_unknown_curators():
    with pytest.raises(ValueError, match="Unknown modality curators"):
        curate_by_modality(
            pd.DataFrame({"patient_id": ["p1"], "Modality": ["CT"]}),
            curators=["builtin:typo"],
        )


def test_router_rejects_unknown_mri_contextual_strategies():
    with pytest.raises(ValueError, match="Unknown MRI contextual strategies"):
        curate_by_modality(
            pd.DataFrame(
                {
                    "patient_id": ["p1"],
