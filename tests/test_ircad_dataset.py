...
def test_ircad_clean_matches_golden(tmp_path, ircad_dicom_root, ircad_reference_csv):
    ...
    manifest = load_manifest(
        "generic", base_path=Path(__file__).resolve().parents[1] / "src" / "imperandi"
    )
    ...
    generated = generated.sort_values("patient_key").reset_index(drop=True)
    golden = golden.sort_values("patient_key").reset_index(drop=True)

    comparable_cols = sorted(
        set(generated.columns)
        .intersection(golden.columns)
        .difference({"volume_id", "ImageOrientationPatient"})
    )

    pd.testing.assert_frame_equal(
        generated[comparable_cols],
        golden[comparable_cols],
        check_dtype=False,
        check_exact=False,
        rtol=1e-6,
        atol=1e-6,
